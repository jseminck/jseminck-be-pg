...
export default class PGModel {
    ...
    async findAll({column, value, limit, orderBy} = {}) {
        const query = stripAdditionalWhitespaces(this.getFindQuery({column, value, limit, orderBy}));
    ...
    getFindQuery({column, value, limit, orderBy}) {
        let query;
        if (!column || !value) {
